...
function findWasmBinary() {
    return locateFile('rtxui_example_loop_complex.wasm');
}
...
var ASM_CONSTS = {
  263904: ($0) => { window.rtxui_has_active_transitions = $0; },  
 263950: () => { return Asyncify.handleSleep(function(wakeUp) { if (window.rtxui_input_queue && window.rtxui_input_queue.length > 0) { wakeUp(window.rtxui_input_queue.shift()); return; } let timer = null; if (window.rtxui_has_active_transitions) { timer = setTimeout(function() { window.rtxui_on_input = null; wakeUp(-1); }, 16); } window.rtxui_on_input = function(char_code) { if (timer) clearTimeout(timer); window.rtxui_on_input = null; wakeUp(char_code); }; }); },  
 264403: ($0) => { let s = UTF8ToString($0); if (window.rtxui_on_output) { window.rtxui_on_output(s); } else { console.log(s); } },  
 264517: () => { return window.rtxui_columns || 80; },  
 264556: () => { return window.rtxui_lines || 24; }
};
