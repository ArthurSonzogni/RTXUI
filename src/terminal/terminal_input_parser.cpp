...
Event TerminalInputParser::MakeSpecialEvent(std::string sequence) {
  auto it = g_uniformize.find(sequence);
  if (it != g_uniformize.end()) {
    return Event::Special(it->second);
  }
  return Event::Special(sequence);
}
...
void TerminalInputParser::Timeout(int time) {
  ...
  timeout_ = 0;
  if (!pending_.empty()) {
    Send(MakeSpecialEvent(pending_));
  }
}
...
TerminalInputParser::Output TerminalInputParser::Parse() {
  ...
  if (Current() < 32) {  // C0 NOLINT
    return MakeSpecialEvent(pending_);
  }
...
TerminalInputParser::Output TerminalInputParser::ParseUTF8() {
  ...
  return Event::Keyboard{.codepoint = value};
}
...
TerminalInputParser::Output TerminalInputParser::ParseMouse(  // NOLINT
  ...
  Event::Mouse mouse;
  mouse.motion = Event::Mouse::Motion(pressed);  // NOLINT
  ...
  // clang-format on
  mouse.motion = is_move ? Event::Mouse::Moved : Event::Mouse::Motion(pressed);
  mouse.button = is_wheel ? Event::Mouse::Button(Event::Mouse::WheelUp + button)  //
                                 : Event::Mouse::Button(button);
  mouse.shift = is_shift;
  mouse.meta = is_meta;
  mouse.control = is_control;
  mouse.x = arguments[1];  // NOLINT
  mouse.y = arguments[2];  // NOLINT
...
TerminalInputParser::Output TerminalInputParser::ParseCursorPosition(
  ...
  cursor_position.y = arguments[0];  // NOLINT
  cursor_position.x = arguments[1];  // NOLINT
