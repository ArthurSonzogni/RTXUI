...
std::shared_ptr<PhysicalFragment> LayoutInlineFlow(
  ...
  if (box->dom_node) {
    // Baseline: the widest single line / full stack of lines, which already
    // captures overflow from unwrapped content (e.g. white-space:nowrap
    // text wider than a fixed width) even when container_frag->width/height
    // were clamped smaller by an explicit size. Mirrors how LayoutBlockFlow
...
std::shared_ptr<PhysicalFragment> LayoutGrid(
  ...
  if (box->dom_node) {
    // Baseline: the natural (unclamped) track extent, which already
    // captures overflow from tracks/content needing more space than an
    // explicit width/height allowed - mirrors LayoutBlockFlow's
    // max_child_width/cur_y-seeded baseline and LayoutInlineFlow's
    // max_line_width/cursor_y-seeded one above.
    int total_scroll_width =
        std::max(container_frag->width, content_w + padding_h + border_h);
    int total_scroll_height =
