...
auto CodePointToString(uint32_t codepoint) -> std::string {
  auto btc = [](uint32_t cp) -> char { return static_cast<char>(cp); };

  if (codepoint <= 0x7F) {
    std::string result;
    result += btc(codepoint);
    return result;
  }

  if (codepoint <= 0x7FF) {
    std::string result;
    result += btc(0b1100'0000 | ((codepoint >> 6) & 0b0001'1111));
    result += btc(0b1000'0000 | (codepoint & 0b0011'1111));
    return result;
  }

  if (codepoint <= 0xFFFF) {
    std::string result;
    result += btc(0b1110'0000 | ((codepoint >> 12) & 0b0000'1111));
    result += btc(0b1000'0000 | ((codepoint >> 6) & 0b0011'1111));
    result += btc(0b1000'0000 | (codepoint & 0b0011'1111));
    return result;
  }
  ...
  if (codepoint <= 0x10FFFF) {
    std::string result;
    result += btc(0b1111'0000 | ((codepoint >> 18) & 0b0000'0111));
    result += btc(0b1000'0000 | ((codepoint >> 12) & 0b0011'1111));
    result += btc(0b1000'0000 | ((codepoint >> 6) & 0b0011'1111));
    result += btc(0b1000'0000 | (codepoint & 0b0011'1111));
    return result;
  }
