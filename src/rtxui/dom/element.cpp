...
std::optional<Color> InterpolateOptionalColor(std::optional<Color> start, std::optional<Color> target, float progress) {
...
const TransitionConfig* FindTransitionConfig(const Element* element, std::string_view property) {
  for (const auto& config : element->target_style.transitions) {
    ...
    if (config.property == "border-color" && property.starts_with("border-") && property.ends_with("-color")) {
      return &config;
    }
  }
  return nullptr;
...
void Element::TriggerTransitions(double current_time_ms) {
  ...
  auto HandleColorProperty = [&](std::string_view prop_name,
    ...
    if (has_active) {
      target_changed = (active_transitions[std::string(prop_name)].target_color != target_val.value_or(Color::RGBA(0, 0, 0, 0)));
    } else {
    ...
    if (target_changed) {
      ...
      if (config && config->duration_seconds > 0.0f) {
        ...
        if (!current_val && target_val) {
          trans.start_color = Color::RGBA(target_val->r, target_val->g, target_val->b, 0);
        }
        trans.target_color = target_val.value_or(Color::RGBA(0, 0, 0, 0));
        if (current_val && !target_val) {
          trans.target_color = Color::RGBA(current_val->r, current_val->g, current_val->b, 0);
        }
  ...
  auto HandleFloatProperty = [&](std::string_view prop_name,
                                 float& current_val,
                                 float target_val) {
    bool has_active = active_transitions.count(std::string(prop_name)) > 0;
    bool target_changed = false;
    if (has_active) {
      target_changed = (active_transitions[std::string(prop_name)].target_float != target_val);
    } else {
      target_changed = (current_val != target_val);
    }
  ...
  auto HandleLengthProperty = [&](std::string_view prop_name,
    ...
    if (has_active) {
      ...
      target_changed = (active.target_length.value != target_val.value || active.target_length.unit != target_val.unit);
    } else {
      target_changed = (current_val.value != target_val.value || current_val.unit != target_val.unit);
    }
  ...
  HandleColorProperty("background-color", style.background_color, target_style.background_color);
  HandleColorProperty("color", style.foreground_color, target_style.foreground_color);
  HandleColorProperty("foreground-color", style.foreground_color, target_style.foreground_color);

  HandleColorProperty("border-top-color", style.border_color_top, target_style.border_color_top);
  HandleColorProperty("border-right-color", style.border_color_right, target_style.border_color_right);
  HandleColorProperty("border-bottom-color", style.border_color_bottom, target_style.border_color_bottom);
  HandleColorProperty("border-left-color", style.border_color_left, target_style.border_color_left);
  ...
  HandleFloatProperty("flex-shrink", style.flex_shrink, target_style.flex_shrink);
}
