...
struct Tree {
  ...
  std::map<std::string_view, Ref<Component>> components;
};
...
Register::Register(Component* component) {
  std::cerr << "Register: " << component->Tag() << " in "
            << Join(component->Namespaces(), ".") << std::endl;
...
// static
const Component* Register::Get(std::string_view name,
